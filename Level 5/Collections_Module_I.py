...
def main():
    ...
    #   A customer wants to buy chocolates from M queries.
    #
    # Input Format:
    ...
    #     add x to total revenue and reduce availability of that choc_type by 1.
    ...
    # Process M customer queries
    for _ in range(M):
        x, choc_type = map(int, input().split())
        if shop_count[choc_type] > 0:  # If chocolate available
            total += x                 # Add revenue
            shop_count[choc_type] -= 1 # Reduce stock
